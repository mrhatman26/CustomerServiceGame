if (queue_pos_updated == true){
	...
	if (my_queue_pos >= 0){
		...
		target_pos = 0 - sprite_get_width(spr_customers_old);
	}
	...
	if (my_queue_pos == 0){
		...
		if (asked_question == false){
			if (mouse_overlaps_me(x, y, spr_customer, size_increase) && mouse_check_button_pressed(mb_left)){
				global.questioning_customer = id;
				global.asking_question = true;
			}
		}
	}
	if (queuing == true){
...
if (global.debug == true){
	if (mouse_overlaps_me(x, y, spr_customers_old, size_increase) && mouse_check_button_pressed(mb_middle)){
		queue_paitence = 0;
	}
}